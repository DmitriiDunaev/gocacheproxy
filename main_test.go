...
func TestProxy(t *testing.T) {
	...
	putOut := bytes.Repeat([]byte{0xCC}, 32)
	putBody := []byte("freshly-produced-output")
	...
	input := buildInput(t, []any{
		...
		request{ID: 3, Command: cmdPut, OutputID: putOut, BodySize: int64(len(putBody))},
		putBody,
		request{ID: 4, Command: cmdClose},
	})
...
func assertPut(t *testing.T, r response, tmpDir string, outID, body []byte) {
	t.Helper()

	want := filepath.Join(tmpDir, hex.EncodeToString(outID)+"-d")
	if r.DiskPath != want {
		t.Fatalf("put: diskPath %q want %q", r.DiskPath, want)
	}

	got, err := os.ReadFile(r.DiskPath)
	if err != nil {
		t.Fatalf("put: read body: %v", err)
	}
